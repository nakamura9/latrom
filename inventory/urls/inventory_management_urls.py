...
inventory_management_urls = [
    re_path(r'^config-wizard', views.ConfigWizard.as_view(), 
        name='config-wizard'),
    re_path(r'^order-stock-receipt-create/(?P<pk>[\d]+)/?$', 
        views.OrderStockReceiptCreateView.as_view(), 
        name="order-stock-receipt-create"),
    re_path(r'^order-stock-receipts-list/(?P<pk>[\d]+)/?$', 
        views.OrderGoodsReceiptsList.as_view(), 
        name="order-stock-receipts-list"),
    ...
    re_path(r'^inventory-check-list/(?P<pk>[\w]+)/?$', 
        views.InventoryCheckListView.as_view(), 
